...
async function main() {
  ...
  // 1. POST Bookmark
  console.log("\n1. Testing POST /api/v1/bookmarks for Question 22...");
  const postRes = await fetch("https://rktechgovtwebsite.onrender.com/api/v1/bookmarks", {
    method: 'POST',
    headers,
    body: JSON.stringify({ questionId: 22 })
  });
  ...
  // 3. DELETE Bookmark
  console.log("\n3. Testing DELETE /api/v1/bookmarks/22...");
  const deleteRes = await fetch("https://rktechgovtwebsite.onrender.com/api/v1/bookmarks/22", {
    method: 'DELETE',
    headers
  });
